...
### 2026-06-29 - macOS 跨端与 UI 自适应边界

- 用户认可/要求保留：Mineradio 从纯 Windows 项目转向跨端（macOS兼容）后，需保持统一的跨端 UI 架构并严禁使用写死的系统级路径。
- 涉及文件：`desktop/main.js`、`desktop/preload.js`、`public/index.html`、`server.js`。
- 关键参数/实现：
  1. **UI 自适应**：Mac 窗口使用 `titleBarStyle: 'hiddenInset'`。通过 `desktop/preload.js` 向前端注入 `document.body.classList.add('macos-shell')`。前端 CSS 使用 `body.macos-shell` 隐藏 Windows 右上角自定义控制按钮，并在 `#desktop-titlebar` 左侧保留 78px 供原生红绿灯使用。
